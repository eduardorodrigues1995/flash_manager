...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $situacao_cnpj = $_POST["situacao_cnpj"];
    ...
    $stmt = $conn->prepare("INSERT INTO pessoa_juridica (cnpj, razao_social, situacao_cnpj, data_abertura, natureza_juridica, capital_social, telefone, email, instagram, rua, n_complemento, bairro, uf, cidade, cep) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    if ($stmt) {
        // Vincular os parâmetros da consulta
        $stmt->bind_param("sssssssssssssss", $cliente_empresa_cnpj, $cliente_empresa_razao_social, $situacao_cnpj, $cliente_empresa_data_abertura, $cliente_empresa_natureza_juridica, $cliente_empresa_capital_social, $cliente_empresa_telefone, $cliente_empresa_email, $cliente_empresa_instagram, $cliente_empresa_rua, $cliente_empresa_n_complemento, $cliente_empresa_bairro, $cliente_empresa_uf, $cliente_empresa_cidade, $cliente_empresa_cep);
